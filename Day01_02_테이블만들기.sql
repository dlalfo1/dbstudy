...
/* 
    ...
    1. CHAR(size)    : 고정 길이 문자 타입(size : 1 ~ 2000바이트)
        - 오라클에선 한글자든 여러글자든 나눠서 저장하지 않는다. (JAVA에서 String개념이라고 생각하면 된다.)
        - 길이가 고정적일 때 사용한다(ex. 주민등록번호)
    ...
    5. NUMBER(p,s)   : 정밀도(p), 스케일(s)로 표현하는 숫자 타입
        ...
        예시)
            (1) NUMBER      : 최대 38자리 숫자(22바이트) - 소숫점이 들어갈 수 있는가
            (2) NUMBER(3)   : 정수부가 최대 3자리인 숫자(최대 999)
            (3) NUMBER(5,2) : 최대 전체 5자리, 소수부 2자리인 숫자 ex)23.45
