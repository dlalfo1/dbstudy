...
SELECT TRUNC(TO_DATE(HIREDATE, 'YYYY/MM/DD') - TO_DATE(SYSDATE, 'YYYY/MM/DD')) AS 근무일
  FROM PROFESSOR
...
SELECT ROW_NUMBER() OVER(ORDER BY (P.SAL) DESC) AS 연봉순위, P.PNAME AS 교수명, P.SAL AS 연봉
  FROM PROFESSOR P;
...
SELECT S.GRADE AS 학년, ROUND(AVG(S.HEIGHT),1) AS 평균키 ,ROUND(AVG(S.WEIGHT),1) AS 평균몸무게
  FROM STUDENT S
GROUP BY S.GRADE
ORDER BY S.GRADE;
...
SELECT GET_STUDENT('mandu') AS 학번
  FROM STUDENT
 WHERE USERID = 'mandu'; 
