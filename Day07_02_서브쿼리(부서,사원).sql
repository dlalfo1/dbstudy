...
/* SELECT절의 서브쿼리 */ -- 스칼라 서브쿼리라고도 불린다.

/*
    스칼라 서브쿼리
    1. SELECT절에서 하나의 값을 반환하는 서브쿼리이다.
    2. 일치하지 않는 정보는 NULL값을 반환한다.
    3. 유사한 방식의 조인 방은 외부조인이다. => 즉, 외부조인으로 대부분 해결 가능하다.
*/
...
SELECT
     ...
     , (SELECT D.DEPT_NAME
         ...
         WHERE D.DEPT_NO = E.DEPART
           AND D.DEPT_NO = 1)
  FROM 
       EMPLOYEE_TBL E; -- 메인쿼리가 바뀌면 서브쿼리도 바뀐다..?

-- 참고. 조인으로 풀어보기
SELECT E.EMP_NO, E.NAME, E.DEPART, D.DEPT_NAME
  FROM DEPARTMENT_TBL D RIGHT OUTER JOIN EMPLOYEE_TBL E -- 일치하는 정보가 없더라고 EMPLOYEE_TBL은 조회를 해달라.
