...
CREATE TABLE CUSTOMER_TBL( 
 ...
 COSTOMER_NAME  VARCHAR2(20 BYTE),
...
INSERT INTO BOOK_TBL(BOOK_ID, BOOK_NAME, PUBLISHER, PRICE) VALUES (BOOK_SEQ.NEXTVAL, '연도 단계별 기술', '굿스포츠', 6000);
...
INSERT INTO BOOK_TBL(BOOK_ID, BOOK_NAME, PUBLISHER, PRICE) VALUES (BOOK_SEQ.NEXTVAL, '올림픽 챔피억', '나이스북', 13000);
COMMIT;
...
RENAME COLUMN COSTOMER_NAME TO CUSTOMER_NAME;
...
ALTER TABLE ORDER_TBL
  ADD CONSTRAINT PK_ORDER으로 PRIMARY KEY(ORDER_ID);
...
-- HAVING COUNT(O.ORDER_ID) = 0; -- HAVING절로 옵션주는걸로 풀 수도 있다.
...
-- 표준 문법
SELECT C.CUSTOMER_NAME AS 고객명, D(DISTINCT B.PUBLISHER) AS 출판사수
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
...
SELECT O.ORDER_ID AS 구매번호
  ...
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
 WHERE TO_DATE(ORDER_DATE, 'YY/MM/DD') NOT BETWEEN TO_DATE('20/07/04', 'YY/MM/DD') AND TO_DATE('20/07/07', 'YY/MM/DD');
...
SELECT O.ORDER_ID AS 구매번호
 ...
 WHERE C.CUSTOMER_ID = O.CUSTOMER_ID 
   AND O.BOOK_ID = B.BOOK_ID
   AND TO_DATE(ORDER_DATE, 'YY/MM/DD') NOT BETWEEN TO_DATE('20/07/04', 'YY/MM/DD') AND TO_DATE('20/07/07', 'YY/MM/DD');
...
-- 표준 문법
SELECT C.CUSTOMER_NAME, NVL(SUM(B.PRICE * O.AMOUNT), 0), COUNT(O.ORDER_ID) AS 구매횟수
  FROM CUSTOMER_TBL C LEFT OUTER JOIN ORDER_TBL O
...
-- 13. 가장 최근에 구매한 고객의 이름과 구매내역(책이름, 주문일자)을 조회하시오. - -여기부터 서브쿼리인듯?
-- 고객명  책이름            주문일자
...
SELECT C.CUSTOMER_NAME AS 고객명, B.BOOK_NAME AS 책이름, O.ORDER_DATE AS 주문일자
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID  
 WHERE ORDER_DATE = (SELECT MAX(ORDER_DATE) FROM ORDER_TBL);
  ...
  FROM BOOK_TBL B LEFT OUTER JOIN ORDER_TBL O
    ...
    ON O.CUSTOMER_ID = C.CUSTOMER_ID
     WHERE B.PRICE = (SELECT MAX(PRICE)
                    FROM BOOK_TBL);
-- WHERE B.PRICE = (SELECT MAX(PRICE) FROM BOOK_TBL); -- 여기서 MAX(B.PRICE)하면 왜 안 되는지..?ㅠㅠ
...
SELECT BB.고객명, BB.총구매액
  FROM (SELECT ROWNUM AS RN, AA.고객명, AA.총구매액
...
WHERE BB.RN BETWEEN 2 AND 3;


-- 2) ROW_NUMBER() 함수
SELECT A.고객명, A.총구매액
  FROM(SELECT ROW_NUMBER() OVER(ORDER BY SUM(B.PRICE * O.AMOUNT) DESC) AS RN, C.CUSTOMER_NAME AS 고객명, SUM(B.PRICE * O.AMOUNT) AS 총구매액
         FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
                    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
                    ON O.BOOK_ID = B.BOOK_ID
