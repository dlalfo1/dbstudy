...
CREATE TABLE CUSTOMER_TBL(
    ...
    CONSTRAINT FK_CUSTOMER FOREIGN KEY(BANK_CODE) REFERENCES BANK_TBL(BANK_CODE)
);

/*
    테이블 변경하기
    1. 칼럼 추가 : ALTER TABLE 테이블명 ADD 칼럼명 데이터타입 [제약조건] -- ADD 뒤 순서는 CREATE 순서와 같다.  
    2. 칼럼 삭제 : ALTER TABLE 테이블명 DROP COLUMN 칼럼명 
    3. 칼럼 수정 : ALTER TABLE 테이블명 MODIFY 칼럼명 데이터타입 [제약조건] (칼럼 이름 수정은 다른 방식으로 한다.)
    4. 칼럼 이름 : ALTER TABLE 테이블명 RENAME COLUMN 기존칼럼명 TO 신규칼럼명
    5. 테이블 이름 : ALTER TABLE 테이블명 RENAME TO 신규테이블명 -- 자주 쓰진 않는다. 실습X
*/
...
-- 2. CUSTOMER_TBL 테이블에서 나이(AGE) 칼럼 삭제하시오.
...
ALTER TABLE BANK_TBL 
    MODIFY BANK_NAME VARCHAR2(15 BYTE) NOT NULL; -- 제약조건만 수정할거니까 기존에 데이터타입까진 그대로 적어준다.
...
ALTER TABLE CUSTOMER_TBL
    MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; -- CAREATE할 때 NULL을 적지않으면 NULL이다. MODIFY할 때 적지 않는 건 이전 제약조건을 바꾸지 않는다는 뜻이다. 
                                             -- 여기선 NULL로 제약조건을 바꾸는 것이기 때문에 NULL 명시를 반드시 해야 한다.
