...
SELECT
       DISTINCT JOB_ID -- JOB_ID 칼럼을 가져올 때 중복제거하고 가져오기.
  FROM
       EMPLOYEES -- EMPLOYEES 테이블에서 가져올 것이다.
 ORDER BY
       JOB_ID ASC; -- ORDER BY 키워드가 정렬해주는 키워드(ASC 키워드는 생략해도 자동으로 오름차순으로 정렬 해준다.)
...
SELECT
  ...
  FROM
       EMPLOYEES
 WHERE
       JOB_ID = 'IT_PROG' OR JOB_ID = 'ST_MAN';
