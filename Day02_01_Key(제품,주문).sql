...
/*
    ...
    1. ON DELETE CASCADE
        ...
        2) 예시) 회원 탈퇴 시 작성한 모든 게시글이 함께 삭제됩니다.
                 게시글 삭제 시 해당 게시글에 달린 모든 댓글이 함께 삭제됩니다. -- 게시글 테이블 만들 때 댓글 테이블도 만들어야 함. 그래야 게시글 삭제시 댓글도 지워지게 만들 수 있다.
                 
    2. ON DELETE SET NULL -- 유연한 처리에 해당된다.
        1) 참조 중인 PARENT KEY가 삭제되면 해당 PARENT KEY를 가진 칼럼 값만 NULL로 처리한다. -- 행 자체가 삭제되는 것은 아니다.
        2) 예시) 어떤 상품을 제거하였으나 해당 상품의 주문 내역은 남아 있는 경우.(제품이 단종된 경우) 
*/
