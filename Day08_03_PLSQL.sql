/*
      ...
      3. 변수에 저장된 값을 출력을 위해서 서버 출력을 ON 시켜줘야 한다.
         최초 1번만 실행시켜 두면 된다. (디폴트는 OFF)
          SET SERVEROUTPUT ON; => 이걸 제일 먼저 해줘야 한다.    
*/

-- 서버출력 ON
SET SERVEROUTPUT ON; -- 실행시켜도 메세지는 뜨지 않는다.

-- 서버 출력 확인
BEGIN 
   DBMS_OUTPUT.PUT_LINE('Hello World'); -- JAVA prinln() 메소드처럼 출력하고 줄 바꿔준다.
END;   
...
CREATE TABLE EMPLOYEES
    AS(SELECT *
         FROM HR.EMPLOYEES); -- 테이블 복사할 때 사용하는 서브쿼리
...
/*       
    ...
    2. 종류
        ...
        2) 참조 변수   : 특정 칼럼의 타입을 겨자다가 지정한다.
        3) 레코드 변수 : 2개 이상의 칼럼을 합쳐셔 하나의 타입으로 지정한다.
...
BEGIN
   SELECT FIRST_NAME, LAST_NAME, SALARY -------
     INTO EMP -- FIRST_NAME, LAST_NAME, SALARY 세개의 칼럼을 저장한다.
     FROM EMPLOYEES
...
BEGIN 
    -- SELECT FIRST_NAME, LAST_NAME, SALARY 이렇게 테이블의 칼럼을 골라서 사용할 수 없다. 싹 다 넣어야한다.
...
BEGIN 
   ...
   ELSIF SCORE >= 60 THEN
          GRADE := 'D';  
   ELSE
       GRADE := 'F'; -- ELSE문에는 조건적지 않는다.
   END IF;
   DBMS_OUTPUT.PUT_LINE(SCORE || '점은' || GRADE || '학점이다');
END;
...
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE; -- EMPLOYEES 테이블의 EMPLOYEE_ID 데이터와 같은 타입이란 소리
    SAL EMPLOYEES.SALARY%TYPE;
...
BEGIN
   FOR N IN 1..10 LOOP
        SELECT MOD(N, 3) -- N을 3로 나눈 나머지를 구하는 함수
          INTO MODULAR -- MOD로 구한 값은 MODULAR에 저장해라
          FROM DUAL;
        IF MODULAR = 0 THEN
             MESSAGE := '3의배수';
        ELSE
...
BEGIN 
    SELECT FIRST_NAME
      ...
      WHERE EMPLOYEE_ID = 0;
EXCEPTION
     WHEN OTHERS THEN -- 모든 예외를 처리할 수 있다.
