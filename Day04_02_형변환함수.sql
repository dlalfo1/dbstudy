...
-- '100' -> 100
SELECT '100',TO_NUMBER('100') -- 문자는 왼쪽정렬, 숫자는 오른쪽정렬이다.
  FROM DUAL;                                   
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES
 WHERE EMPLOYEE_ID = '150'; -- EMPLOYEE_ID = TO_NUMBER('150')으로 자동으로 변한한 뒤 처리한다.
...
SELECT
     ...
     , TO_DATE('05/06/07', 'MM/DD/YY') -- 내가 TO_DATE함수로 월/일/년 을 만들어줌 하지만 DB에서 출력은 
                                       -- 연/월/일로 나오기 때문에  07/05/06
  FROM
       DUAL;
