...
ALTER TABLE BUYS
    ADD CONSTRAINT FK_BUYS_USERS FOREIGN KEY(USER_ID)
        REFERENCES USERS(USER_ID); -- USERS 테이블의 USER_ID가 기본키가 아닌데 외래키를 맺을 수 있는 이유는 UNIQEU 특성 때문이다.
-- 구매-제품 외래키
ALTER TABLE BUYS
    ADD CONSTRAINT FK_BUYS_PRODUCTS FOREIGN KEY(PROD_CODE) -- PRODUCTS테이블의 기본키를 참조함.
...
SELECT USER_NO, USER_ID, USER_MOBILE1, USER_MOBILE2
  FROM USERS
 WHERE USER_MOBILE1 IS NULL; -- 값이 없다, 빈칸이다라는 IS NULL 키워드를 사용해야한다.

-- 5. 연락처2가 '5'로 시작하는 사용자의 사용자번호, 아이디, 연락처1, 연락처2를 조회하시오.
SELECT USER_NO, USER_ID, USER_MOBILE1, USER_MOBILE2
  FROM USERS
WHERE USER_MOBILE2 LIKE '5%'; -- 어떤 단어로 시작한다~ 할 때 % 사용시 앞에 LIKE 키워드 사용한다.( = 등호 아님 주의!)


-- 6. 제품을 구매한 사용자의 아이디별 구매횟수를 조회하시오. => 동일한 ID는 하나로 묶어야함 (GROUP BY)
-- 아이디  구매횟수
-- KHD     3
-- PSH     3
-- KYM     1
...
SELECT U.USER_ID AS 아이디, U.USER_NAME AS 고객명, COUNT(BUY_NO) AS 구매횟수 -- 구매내역이 없는 사람은 BUY_NO로 집계해준다.
                                                                                     -- BUYS 테이블에서            
...
SELECT U.USER_ID AS 아이디, U.USER_NAME AS 고객명, COUNT(*) -- COUNT(*) 함수 사용시 값이 NULL이여도 1번으로 카운팅한다.
                                                            -- 그렇기 때문에 BUY_NO으로 카운팅해준다.                                                                                  
  FROM USERS U LEFT OUTER JOIN BUYS B
    ON U.USER_ID = B.USER_ID
 GROUP BY U.USER_ID, U.USER_NAME
 ORDER BY U.USER_ID; 
...
SELECT U.USER_NAME AS 고객명, P.PROD_NAME AS 구매제품
  FROM USERS U LEFT OUTER JOIN BUYS B
    ON U.USER_ID = B.USER_ID LEFT OUTER JOIN PRODUCTS P -- 2번째 JOIN 할 때도 사용자 내역을 다 보여주기 위해서 LEFT OUTER JOIN 사용
 ...
 SELECT U.USER_NAME AS 고객명
        , (SELECT P.PROD_NAME
             FROM PRODUCTS P
            WHERE P.PROD_CODE = B.PROD_CODE) -- 집가서 해보기
   FROM USERS U LEFT OUTER JOIN BUYS B  
     ON U.USER_ID = B.USER_ID
  ORDER BY U.USER_ID; 
